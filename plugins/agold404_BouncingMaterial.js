...
{
const funcKey='test_bouncingMaterial';
getTopFrameWindow()[funcKey]=window[funcKey]=(i,opt)=>{
let url='img/pictures/$elephant.png?scale=-1,1';
{
url='img/titles1/Plain.png';
//const sp=window._sp=new Sprite_BouncingMaterial_root(ImageManager.loadNormalBitmap(url),opt,new Rectangle(100,100,400,400));
//SceneManager._scene.addChild(sp);
const sp=SceneManager._scene.children[0].children[0];
let sels;
let lastXY;
SceneManager.additionalUpdate_updateScene_add(()=>{
	if(!(SceneManager._scene instanceof Scene_Title)) return true;
	const ti=TouchInput;
	const pt=sp.parent.toLocal({x:ti.x,y:ti.y});
	if(ti.isTriggered()){
		sels=sp.bouncingMaterial_findNearestPoints_parent(pt.x,pt.y).slice(0,1);
		sels[0]._fixedPos=true;
	}
	if(!TouchInput.isPressed()){
		if(sels) sels[0]._fixedPos=false;
		lastXY=
		sels=
		undefined;
		return;
	...
	if(!sels) return;
	if(!lastXY) lastXY={x:pt.x,y:pt.y};
	let dx=pt.x-lastXY.x;
	let dy=pt.y-lastXY.y;
	//console.log(pt.x,pt.y,dx,dy); // debug
	for(let i=0,sz=sels.length;i!==sz;++i){
		sels[i][0]+=dx;
		sels[i][1]+=dy;
		dx>>=1;
		dy>>=1;
...
{
const sp=window._sp=new Sprite_BouncingMaterial_unitRoot(ImageManager.loadNormalBitmap(url),i,new Rectangle(100,100,400,400));
SceneManager._scene.addChild(sp);
let sel=-1;
let lastXY;
SceneManager.additionalUpdate_updateScene_add(()=>{
	if(!(SceneManager._scene instanceof Scene_Title)) return true;
	for(let x=0;x!==4;++x) if(Input.isTriggered(49+x+'')) sel=x;
	if(Input.isTriggered(192+'')) sel=-1;
	if(sel<0) return;
	if(!TouchInput.isPressed()) return lastXY=undefined;
	const TI=TouchInput;
	const ti={x:TI.x,y:TI.y};
	if(lastXY){
		const pt=sp._bouncingMaterial_unitRoot_corners.points[sel];
		let dx=ti.x-lastXY.x;
		let dy=ti.y-lastXY.y;
		const ds=Math.sqrt(dx*dx+dy*dy);
		const maxSpeed=sp.bouncingMaterial_getMaxSpeed();
		if(maxSpeed<ds){
			const r=maxSpeed/ds;
			dx*=r;
			dy*=r;
...
}; // test_bouncingMaterial(0,{nw:23,nh:23});
} // test
